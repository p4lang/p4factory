 ...
 */
static unsigned short vxlan_port __read_mostly = 4790;
		   ...
		   __be16 src_port, __be16 dst_port, __be32 vni, bool xnet)
{
	...
	if (attach_int) {
		...
		int_md->ins_cnt = 3;
		int_md->max_hop_cnt = 32;
		int_md->inst_mask = htons(0xB000);
		int_md->total_hop_cnt = 0;

		/* Insert INT shim header */
		int_sh = (struct int_shim_hdr *) skb_push(skb, sizeof(*int_sh));
		memset(int_sh, 0, sizeof(*int_sh));
		int_sh->length = 3; /* this header (1) + INT meta data header (2) */
...
static int __init vxlan_init_module(void)
{
	...
	printk(KERN_INFO "[VXLAN-GPE] Loading VXLAN-GPE driver\n");
