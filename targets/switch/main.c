...
TAILQ_HEAD(if_with_ports_head, with_port_entry) if_with_ports;
struct with_port_entry {
  char *name;
  uint32_t port_num;
  TAILQ_ENTRY(with_port_entry) entries;
};
...
static int bm_pps = 0;
static int bm_qdepth = 0;
static bool bm_trace = false;
...
parse_options(int argc, char **argv)
{
  ...
  struct with_port_entry *wpe = NULL;
  TAILQ_INIT(&if_with_ports);
  ...
  while (1) {
    ...
    case OPT_OFIPV6:
      ...
      break;
    case OPT_IFPORT:
      wpe = malloc(sizeof(struct with_port_entry));
    ...
    case '?':
      ...
      printf("--ifport=INTERFACE:PORT Allows you to manually specify a port number for an interface"),
      printf(" --name=NAME Set the name of the datapath (default p4ns)\n");
      printf(" --dpid=DPID Set datapath ID (default autogenerated)\n");
      printf(" --pps=PPS Set max packets/sec rate\n");
      printf(" --qdepth=Qlen Set max Q depth\n");
...
main(int argc, char* argv[])
{
    ...
    if (bm_trace) {
      ...
      rmt_log_level_set(P4_LOG_LEVEL_ERROR);
    }
    ...
    if (listener_str) {
        ...
        /* Add interfaces specified with ports */
        struct with_port_entry *wpe;
        for (wpe = if_with_ports.tqh_first; wpe != NULL; wpe = wpe->entries.tqe_next) {
            printf("Adding interface %s (port %d)\n", wpe->name, wpe->port_num);
            if(add_port(wpe->name, wpe->port_num) < 0) {
                printf("Failed to add interface %s\n", wpe->name);
                return -1;
            }
            p4ns_db_add_port(c, datapath_name, wpe->name, wpe->port_num);
            port_no++;
        }
    ...
    // Set pps and qdepth parameter for BM
    // 0 has no effect on default settings
    set_packets_per_sec(bm_pps);
    set_drop_tail_thr(bm_qdepth);
