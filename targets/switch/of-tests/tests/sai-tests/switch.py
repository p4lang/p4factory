...
class CpuNetdevTest(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        print
        switch_init(self.client)
        port1 = port_list[1]
        ...
        queue_id = 1
        trap_group_priority = 1000
        trap_group_id = sai_thrift_create_hostif_trap_group(self.client, queue_id, trap_group_priority)
        action = 2
        priority = 1000
        channel = 2
        sai_thrift_create_hostif_trap(self.client, 0x2000, action, priority, channel, trap_group_id)
        sai_thrift_create_hostif_trap(self.client, 0x2001, action, priority, channel, trap_group_id)
        sai_thrift_create_hostif_trap(self.client, 0x4001, action, priority, channel, trap_group_id)

        intf_name1 = 'swp1'
        hif_id = sai_thrift_create_hostif(self.client, rif_id1, intf_name1)
        ...
        try:
            ...
            pkt = simple_arp_packet(eth_dst='00:77:66:55:44:33',
                                eth_src='00:0b:00:00:00:01',
                                arp_op=2)
            self.dataplane.send(1, str(pkt))
            ...
            self.client.sai_thrift_remove_hostif(hif_id)
            self.client.sai_thrift_remove_hostif_trap_group(trap_group_id)
            self.client.sai_thrift_remove_router_interface(rif_id1)
            self.client.sai_thrift_remove_virtual_router(vr_id)
